...
def import_project_plot_style(workspace: Path):
    ...
    try:
        ...
        def format_te_axis(ax, show_grid: bool = False, tick_labelsize: int = 9) -> None:
            ...
            if show_grid:
                ax.grid(True, which="major", linestyle="--", linewidth=0.4, alpha=0.45)
            else:
                ax.grid(False)

        def save_figure(fig, save_name: str, save_dir: str = "results/plots", formats=None, transparent: bool = False):
...
def display_path_for(workspace: Path, path: Path) -> str:
    try:
                ...
                resolved.append(
                    {
                        ...
                        "sample_composition": "",
                        "processed_file": display_path_for(workspace, csv_path),
                    }
                )
...
def collect_points(
    ...
    for index, meta in enumerate(rows):
        ...
        record: dict[str, Any] = {
            ...
            "composition": meta.get("sample_composition", ""),
            "processed_file": display_path_for(workspace, csv_path_for(workspace, meta)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="results/plots/paper_style_variants")
