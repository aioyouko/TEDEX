...
try:
    ...
    def save_figure(fig, save_name, save_dir="results/plots", formats=None, transparent=False):
...
DEFAULT_XRD_SAVE_DIR = Path("results/plots/xrd")
...
def plot_xrd_comparison(
    ...
    for index, (pattern, x_values, y_values) in enumerate(prepared):
        ...
        color = next(colors)
        (line,) = ax.plot(x_values, y_values + offset, color=color, linewidth=1.2, label=pattern.label)
        handles.append(line)
    ...
    if pdf_standard is not None:
        ...
        if not peaks.empty:
            ...
            handles.append(
                plt.Line2D([0], [0], color=standard_color, linewidth=1.8, label=pdf_standard.label)
            )
            labels.append(pdf_standard.label)

    ax.set_xlabel(r"2$\mathit{Theta}$ (Degree)", fontsize=22)
    ylabel = r"$\mathit{Intensity}$ (a.u.)" if normalized else r"$\mathit{Intensity}$ (counts)"
    ax.set_ylabel(ylabel, fontsize=22)
    if two_theta_range:
        ax.set_xlim(*two_theta_range)
    ax.set_yticks([])
    format_te_axis(ax, show_grid=False, tick_labelsize=14)
    ax.tick_params(axis="x", which="both", direction="out", top=False, bottom=True, labelsize=14, pad=8)
