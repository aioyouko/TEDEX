...
def load_lab_ledger(file_path="configs/lab_batches.json"):
    ...
    if (
        flat_ledger
        or reference_ledger
        or Path("data/lab/batches.json").exists()
        or Path("data/lab/samples.json").exists()
        or Path("data/reference/samples.json").exists()
    ):
...
def get_batch_plot_dir(batch_id):
    return os.path.join("results", "plots", batch_id)
...
def get_interbatch_plot_dir(comparison_id):
    return os.path.join("results", "plots", "inter-batch", comparison_id)
...
def parse_args():
    ...
    display_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Display each generated figure with matplotlib after saving.",
    )
    display_group.add_argument(
        "--no-show",
        dest="show",
        action="store_false",
        help="Save figures without opening a matplotlib preview window. Default.",
    )
...
def main():
    ...
    if not target_batches:
        print("No TE data selected. Pass a batch, sample, processed CSV, or processed CSV folder.")
        return
