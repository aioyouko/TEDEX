...
def load_lab_ledger(file_path="configs/lab_batches.json"):
    ...
    if flat_ledger or Path("data/lab/batches.json").exists() or Path("data/lab/samples.json").exists():
        return flat_ledger
...
def sync_lab_metadata_from_markdown():
    if not LAB_MARKDOWN_PATH.exists():
        print("No data/lab/lab_metadata.md found; skipping Markdown import.", flush=True)
        return

    markdown_text = LAB_MARKDOWN_PATH.read_text(encoding="utf-8")
    ...
    if not has_records:
        print("data/lab/lab_metadata.md has no batch/sample records; skipping Markdown import.", flush=True)
        return
...
def execute_selected_batches(
    ...
    if not ALL_LAB_BATCHES:
        print("No lab batches configured yet. Add raw data under data/raw, then run scripts/sync_lab_metadata.py.")
        return

    selected_batches = resolve_target_batches(normalized_targets, analyze_all=analyze_all)
    if not selected_batches:
        print("No batches selected. Pass a batch id such as CHY-1051, or use --all.")
        return
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="*",
        help=(
            "Batch ids to analyze. Examples: CHY-1051, 1051, CHY-1038 CHY-1040. "
            "If omitted, no analysis is run unless --all is used."
        ),
    )
    parser.add_argument(
        "--all",
